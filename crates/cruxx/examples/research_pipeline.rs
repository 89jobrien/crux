/// Research pipeline: a full end-to-end cruxx agent.
///
...
#[tokio::main]
async fn main() {
    let query = "cruxx agentic DSL";
    let cruxx = run_pipeline(query).await;

    println!("Query:  {query}");
    println!("Output: {}", cruxx.value().unwrap());
    println!();
    println!(
    ...
    println!();
    if let Some(child) = cruxx.children.first() {
