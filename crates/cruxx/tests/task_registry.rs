...
use cruxx::prelude::*;
use cruxx::registry::InMemoryBackend;
...
async fn full_task_lifecycle() {
    ...
    // Execute agent.
    let cruxx = adder(7).await;
    assert_eq!(*cruxx.value().unwrap(), 22); // 7 + 10 + 5
...
async fn resume_from_checkpoint_replays_steps() {
    ...
    // First run.
    let cruxx = adder(3).await;
    assert_eq!(*cruxx.value().unwrap(), 18); // 3 + 10 + 5
    reg.checkpoint(&id, &cruxx).await.unwrap();
...
async fn lenient_replay_skips_removed_step() {
    ...
    let cruxx1 = ctx1.finalize(Ok(25));
