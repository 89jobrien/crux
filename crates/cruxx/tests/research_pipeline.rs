...
async fn pipeline_succeeds_and_produces_output() {
    let cruxx = research_pipeline("cruxx agentic DSL".to_string()).await;
    assert!(cruxx.value().is_ok());
    let output = cruxx.value().unwrap();
    assert!(output.contains("words"));
}

#[tokio::test]
async fn pipeline_records_fetch_steps() {
    let cruxx = research_pipeline("test query".to_string()).await;
    let fetch_steps: Vec<_> = cruxx
        .steps
        .iter()
...
async fn pipeline_has_delegation_child() {
    ...
    assert_eq!(cruxx.children[0].agent, "counter");
}

#[tokio::test]
async fn pipeline_decide_step_recorded() {
    let cruxx = research_pipeline("test query".to_string()).await;
    let decide = cruxx.steps.iter().find(|s| s.name.starts_with("decide::"));
