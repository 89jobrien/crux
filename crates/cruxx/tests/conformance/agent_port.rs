...
async fn conformance_agent_macro_run_returns_correct_value() {
    let cruxx = macro_add(3, 4).await;
    assert_eq!(cruxx.value().unwrap(), &7);
}

#[tokio::test]
async fn conformance_agent_macro_cruxx_agent_field_is_fn_name() {
    let cruxx = macro_add(1, 2).await;
    assert_eq!(cruxx.agent, "macro_add");
}

#[tokio::test]
async fn conformance_agent_macro_steps_recorded_in_trace() {
    let cruxx = macro_add(10, 20).await;
    assert!(
        cruxx.steps.iter().any(|s| s.name == "add"),
...
impl Agent for DoubleAgent {
    ...
    async fn run(ctx: &mut cruxx::ctx::CruxCtx, input: u32) -> Result<u32, CruxErr> {
        ctx.step(
            "double_inner",
...
async fn conformance_agent_handwritten_default_budget_kind_is_tokens() {
    ...
    assert_eq!(budget.kind(), cruxx::types::budget::BudgetKind::Tokens);
