...
/// Register the `llm::extract` handler with the default BAML client.
pub fn register_extract(registry: &mut HandlerRegistry) {
    register_extract_with(registry, None);
}

/// Register the `llm::extract` handler, optionally routing all calls through
/// the given [`ClientRegistry`].
pub fn register_extract_with(
    registry: &mut HandlerRegistry,
    client_registry: Option<ClientRegistry>,
) {
    registry.handler("llm::extract", move |input: Value| {
            ...
            match function.as_str() {
                "ExtractEntities" => {
                    ...
                    let result = if let Some(ref reg) = client_registry {
                        B.ExtractEntities.with_client_registry(reg).call(text).await
                    } else if let Some(ref name) = client_override {
                        B.ExtractEntities.with_client(name).call(text).await
                    } else {
                        B.ExtractEntities.call(text).await
                    }
                    .map_err(|e| {
                ...
                "Summarize" => {
                    ...
                    let result = if let Some(ref reg) = client_registry {
                        ...
                        B.Summarize.call(text, max_sentences).await
                    }
                    .map_err(|e| {
                        CruxErr::step_failed("llm::extract", format!("BAML error: {e}"))
                    })?;
                ...
                "Classify" => {
                    ...
                    let result = if let Some(ref reg) = client_registry {
                        ...
                        B.Classify.with_client(name).call(text, &labels).await
                    } else {
                        B.Classify.call(text, &labels).await
                    }
                    .map_err(|e| {
                        CruxErr::step_failed("llm::extract", format!("BAML error: {e}"))
                    })?;
                ...
                "DescribeProject" => {
                    ...
                    let result = if let Some(ref reg) = client_registry {
                        B.DescribeProject
                            ...
                            .await
                    } else if let Some(ref name_override) = client_override {
                        B.DescribeProject
                            .with_client(name_override)
                            .call(name, language, readme, &commits)
                            .await
                    } else {
                        B.DescribeProject
                ...
                "AssessHealth" => {
                    let name = input_map
                        ...
                        .ok_or_else(|| {
                            CruxErr::step_failed(
                                "llm::extract",
                                "AssessHealth requires 'name' field",
                            )
                        })?
                        .to_string();
                ...
                "GenerateChangelog" => {
                    ...
                    let result = if let Some(ref reg) = client_registry {
                        ...
                        B.GenerateChangelog.call(name, &commits).await
                    }
                    .map_err(|e| {
                        CruxErr::step_failed("llm::extract", format!("BAML error: {e}"))
                    })?;
                ...
                "SuggestRelated" => {
                    let name = input_map
                        ...
                        .ok_or_else(|| {
                            CruxErr::step_failed(
                                "llm::extract",
                                "SuggestRelated requires 'name' field",
                            )
                        })?
                        .to_string();
                    let description = input_map
