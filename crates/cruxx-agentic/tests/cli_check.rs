/// Integration tests for `cruxx check` subcommand.
///
/// Tests write temporary pipeline files and invoke the check logic directly
/// via `cruxx_script::load_file` (same path as the binary) to avoid
...
"#;
    ...
    let result = cruxx_script::load_file(f.path().to_str().unwrap());
