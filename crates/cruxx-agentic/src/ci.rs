...
pub fn register(registry: &mut HandlerRegistry) {
    registry.handler_value(handlers::CI_COMPILE_ERRORS, |input: Value| async move {
    ...
    registry.handler_value(handlers::CI_CLIPPY_VIOLATIONS, |input: Value| async move {
        let log = input.get("log").and_then(|l| l.as_str()).unwrap_or("");
    ...
    registry.handler_value(handlers::CI_NEXTEST_FAILURES, |input: Value| async move {
        let log = input.get("log").and_then(|l| l.as_str()).unwrap_or("");
    ...
    registry.handler_value(handlers::CI_DENY_VIOLATIONS, |input: Value| async move {
        let log = input.get("log").and_then(|l| l.as_str()).unwrap_or("");
        let mut violations = Vec::new();
    ...
    registry.handler_value(handlers::CI_DEDUPLICATE_SPANS, |input: Value| async move {
    ...
    registry.handler_value(handlers::CI_CLASSIFY_SEVERITY, |input: Value| async move {
        let items = input
            .get("items")
    ...
    registry.handler_value(handlers::CI_ATTACH_OWNERS, |input: Value| async move {
        let ranked = input
            .get("ranked")
