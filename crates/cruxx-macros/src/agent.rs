/// Code generation for `#[cruxx::agent]`.
///
/// Transforms:
///   #[cruxx::agent]
...
pub fn expand(attr: TokenStream, item: TokenStream) -> syn::Result<TokenStream> {
    ...
    let replay_setup = match args.replay {
        ...
        crate::parse::ReplayMode::Lenient => quote! {
            __cruxx_ctx.set_replay_mode(::cruxx_core::replay::ReplayMode::Lenient);
        },
    };
    ...
    let registered_impl = if has_registry {
        quote! {
            impl #agent_struct {
                ...
                pub async fn run_registered<B: ::cruxx_core::registry::RegistryBackend>(
                    registry: &::cruxx_core::registry::TaskRegistry<B>,
                    input: <Self as ::cruxx_core::agent::Agent>::Input,
                ) -> (
                    ::cruxx_core::types::crux_value::Crux<<Self as ::cruxx_core::agent::Agent>::Output>,
                    ::cruxx_core::types::id::TaskId,
                )
                where
                    <Self as ::cruxx_core::agent::Agent>::Input: ::serde::Serialize + Clone,
                    <Self as ::cruxx_core::agent::Agent>::Output: ::serde::Serialize,
                {
                    // Submit task
                    ...
                    let _ = registry
                        .update_status(
                            &task_id,
                            ::cruxx_core::registry::TaskStatus::Running,
                        )
                        .await;

                    // Execute
                    let mut __cruxx_ctx = ::cruxx_core::ctx::CruxCtx::new(stringify!(#fn_name));
                    #replay_setup
                    let __cruxx_result = <#agent_struct as ::cruxx_core::agent::Agent>::run(
                        &mut __cruxx_ctx,
                        input,
                    )
                    .await;

                    let cruxx = __cruxx_ctx.finalize(__cruxx_result);

                    // Update final status
                    let final_status = if cruxx.value().is_ok() {
                        ::cruxx_core::registry::TaskStatus::Done
                    } else {
                        ::cruxx_core::registry::TaskStatus::Failed
                    };
                    let _ = registry.update_status(&task_id, final_status).await;

                    // Checkpoint final trace
                    let _ = registry.checkpoint(&task_id, &cruxx).await;
    ...
    Ok(quote! {
        ...
        impl ::cruxx_core::agent::Agent for #agent_struct {
            ...
            async fn run(
                __cruxx_ctx: &mut ::cruxx_core::ctx::CruxCtx,
                #input_destructure: Self::Input,
            ) -> ::core::result::Result<Self::Output, ::cruxx_core::types::error::CruxErr> {
                let x = __cruxx_ctx;
                #fn_block
            }
        }
        ...
        #fn_vis async fn #fn_name(#fn_inputs) -> ::cruxx_core::types::crux_value::Crux<#output_type> {
            let mut __cruxx_ctx = ::cruxx_core::ctx::CruxCtx::new(stringify!(#fn_name));
            #replay_setup
            let __cruxx_result = <#agent_struct as ::cruxx_core::agent::Agent>::run(
                &mut __cruxx_ctx,
            ...
            __cruxx_ctx.finalize(__cruxx_result)
        }
    })
}
...
fn extract_cruxx_inner_type(output: &syn::ReturnType) -> syn::Result<TokenStream> {
    match output {
        syn::ReturnType::Default => Err(syn::Error::new_spanned(
            output,
            "#[cruxx::agent] functions must return Crux<T>",
        )),
