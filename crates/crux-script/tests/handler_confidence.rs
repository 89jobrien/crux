...
async fn low_confidence_handler(
    _input: Value,
) -> Result<HandlerOutput, crux_runtime::prelude::CruxErr> {
...
async fn branch_high(_input: Value) -> Result<HandlerOutput, crux_runtime::prelude::CruxErr> {
...
fn make_registry(
    classify: fn(
        ...
        Box<
            dyn std::future::Future<Output = Result<HandlerOutput, crux_runtime::prelude::CruxErr>>
                + Send,
        >,
    >,
) -> Arc<HandlerRegistry> {
