...
[dependencies]
cruxx       = { version = "0.2", features = ["tokio-runtime", "redb"] }
...
`serde` support is always compiled into `cruxx` — no separate feature flag needed.
...
use cruxx::registry::{RedbBackend, TaskRegistry, TaskStatus};
...
- **`RedbBackend::open(path)`** — the redb adapter takes a `&str` path. There is no SQLite
  integration in cruxx; redb is the embedded persistence option.
- **`run_registered`** — generated by the `registry = "plan"` attribute on `#[cruxx::agent]`.
...
Every structural choice in this system maps to one cruxx primitive:
...
| Registry lifecycle | `#[cruxx::agent(registry = "...")]` + `run_registered` |
...
Chapter **07** compares the cruxx model against existing agentic patterns.
