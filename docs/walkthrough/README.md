...
`crux::` is not a standalone language — it is a Rust DSL (a set of macros, traits, and types)
...
If you have written Rust agents with `tokio` + `tracing` + a hand-rolled task queue, `crux::` is
...
- What `crux::` gives you that a hand-rolled `tokio::spawn` + channels setup does not
