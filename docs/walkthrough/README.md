...
`cruxx::` is not a standalone language — it's a Rust DSL (a set of macros,
...
task queue, `cruxx::` is what happens when you bake those patterns into the
language itself.
...
- What `cruxx::` gives you that a hand-rolled `tokio::spawn` + channels setup
  doesn't
...
| R   | [Syntax reference card](../cruxx-syntax-reference.md)                            | Every macro, trait, and type in one page                                 |
